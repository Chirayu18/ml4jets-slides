...
\institute[]{%
  ...
  \inst{2}Northwestern University}
...
\begin{frame}[plain,noframenumbering]
  ...
  \vspace{-0.6em}
  \begin{center}
    \includegraphics[height=0.085\textheight]{fig/logo_vub.png}\hspace{3em}
    \includegraphics[height=0.085\textheight]{fig/logo_northwestern.png}
  \end{center}
\end{frame}
...
\begin{center}
\includegraphics[height=0.62\textheight]{fig/combined_regimes.png}
\end{center}
\vspace{-0.5em}
\smallnote{\textbf{vectorised} = one FastJet call over the whole batch
(its awkward-array interface); \textbf{one at a time} = a separate
\texttt{ClusterSequence} per jet. Both run FastJet on a \textbf{single CPU
thread}.}
\end{column}
\end{columns}
\end{frame}
...
\begin{center}
\includegraphics[width=\linewidth,trim=0 0 0 0,clip]{fig/atlas_speedup_algs.png}
\end{center}
...
\end{column}
\begin{column}{0.5\textwidth}
\small An \textbf{early gain in every class}, \textbf{gone by 1\,M}. Our conclusions agree with what Sitian presented yesterday(\href{https://indico.global/event/15240/contributions/165243/}{link}) from a \textbf{Jacobian lens} perspective
\end{column}
\end{columns}
\end{frame}
...
\begin{enumerate}
  ...
  left.
  \medskip
  \item \textbf{Public}, as part of \textbf{jet-universe}:
  \texttt{github.com/jet-universe/FlashJet}
\end{enumerate}
\end{frame}
