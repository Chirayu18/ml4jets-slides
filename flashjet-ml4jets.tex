...
triangle --- so the LL predictions are exact.}
\end{column}
\begin{column}{0.49\textwidth}
\realCbody\\
{\small Real detector-level \textbf{calorimeter-cluster constituents} (massless),
CC0, read straight over the public redirector.}
\smallskip
...
interface which is the faster, fairer baseline.}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{Time per unit in both regimes \hfill \realC}
\begin{center}
...
a \emph{jet} (blue) or an \emph{event} (orange) }
\end{frame}
